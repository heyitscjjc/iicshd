...
if(isset($_POST['submitDoc'])){
    ...
    $docStatus = "Received by the Office.";
    $hidden = 0;
    $date = date("Y-m-d H:i:s");
    
	/*$query="INSERT INTO documents VALUES ('', '$date', '$userNo', '$docTitle', '$docDesc', '$docStatus', '$date', '$docISO', '$submittedby', '$hidden')"; 
    $conn->query($query);
	*/
	$submitquery = $conn->prepare("INSERT INTO documents VALUES ('', ?, ?, ?, ?, ?, ?, ?, ?, ?)");
	$submitquery->bind_param("iisssissi", $date, $userNo, $docTitle, $docDesc, $docStatus, $date, $docISO, $submittedby, $hidden);
	$submitquery->execute();
	$submitquery->close();
...
if (isset($_POST['receiveRel'])) {
    ...
    $editquery = $conn->prepare("UPDATE documents SET docstatus=?, hidden=?, docdatechange=NOW() WHERE docno=?");
    $editquery->bind_param("sii", $docstatus, $hidden, $recDoc);
    $editquery->execute();
    $editquery->close();

    $editquery2 = $conn->prepare("UPDATE doclogs SET docstatus=?, hidden=?, docdatechange=NOW() WHERE docno=?");
    $editquery2->bind_param("sii", $docstatus, $hidden, $recDoc);
    $editquery2->execute();
    $editquery2->close();
                ...
                <div class="accordion" id="accordionExample">
					...
					<div class="card">
                        <div class="card-header" id="headingOne1">
                            <h5 class="mb-0">
                                <button class="btn bg-dark text-white" type="button" data-toggle="collapse" data-target="#collapseOne1" aria-expanded="false" aria-controls="collapseTwo">
                                ...
                                <div class="card-body">
                                    <div class="table-responsive">
										...
										<div class="form-group">
                                            <input type="text" class="form-control" id = "docISO" placeholder="Document ISO" value="" name="docISO"/>
                                        </div>
                                        ...
                                        <div class="form-group">
										 ...
										 <div class="form-group">
                                            <textarea requried class="form-control" placeholder="Submitted By:" name="submittedby"></textarea>
                                        </div>
                                        <div class="form-group">
                ...
                <div class="table-responsive">
                    ...
                    <table id="received" class="table table-striped table-responsive-lg">
                        ...
                        <tbody>
                            ...
                            <?php
                            $newsubquery = mysqli_query($conn, "SELECT LPAD(documents.docno,4,0), documents.docdatechange, documents.docdatesubmit, documents.submittedby, documents.docISO, users.userno, users.fname, users.mname, users.lname, documents.doctitle, documents.docdesc, documents.docstatus FROM documents INNER JOIN users "
                                                                ...
                                                                <div class="modal-dialog modal-lg">
                                                                    ...
                                                                    <div class="modal-content">
                                                                        ...
                                                                        <div class="modal-body">
                                                                            <div class="row">
                                                                                <div class="col-sm-12">
																					<input type="hidden" name="edit_doc_no2" value="' . $dociso . '">
